...
fn launch_cosmos_validator(
    ...
    let validator = Program::default()
        ...
        .hyp_env("BASE_TRACING_LEVEL", if debug { "debug" } else { "info" })
        .arg("tracing.level", if debug { "debug" } else { "info" })
        .spawn("VAL");
...
fn launch_cosmos_relayer(
    ...
    let relayer = Program::default()
        ...
        .hyp_env("METRICSPORT", metric.to_string())
        .hyp_env("TRACING_LEVEL", if debug { "debug" } else { "info" })
        .hyp_env("BASE_TRACING_LEVEL", if debug { "debug" } else { "info" })
        .arg("tracing.level", if debug { "debug" } else { "info" })
        .spawn("RLY");
...
fn run_locally() {
    ...
    let nodes = nodes
        ...
        .map(|(launch_resp, chain_id, metric, domain)| {
            ...
            (launch_resp, deployments, chain_id, metric, domain)
        })
        .collect::<Vec<_>>();
    ...
    let hpl_rly_metric_port = metric_start + node_count + 1u32;
    ...
    loop {
        ...
        if termination_invariants_met(hpl_rly_metric_port, dispatched_messages).unwrap_or(false) {
...
fn termination_invariants_met(metrics: u32, messages_expected: u32) -> eyre::Result<bool> {
